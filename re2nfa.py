...
def re2nfa(regex):
    ...
    for i in regex:
        if i in sigma:
            ...
            delta.append({});delta.append({})
            stack.append([operand1_count,operand2_count])
            delta[nfa2]['$']=(nfa1,operand2_count);delta[operand1_count]['$']=(nfa1,operand2_count)
            if initial==nfa1:
            ...
            nfa21,nfa22=stack.pop()
            stack.append([operand1_count,operand2_count])
            delta[operand1_count]['$']=(nfa21,nfa11); delta[nfa12]['$']=operand2_count; delta[nfa22]['$']=operand2_count
...
def eps_closer(nfa, node_set):
    global something_very_different
    try:
